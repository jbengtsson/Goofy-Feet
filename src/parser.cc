...
(const char *name, eval_t fn, gf_expr_type R, unsigned N, va_list args)
  :name(name), fn(fn), result_type(R), arg_types(N)
{
  for(unsigned i=0; i<N; i++)
    arg_types[i] = (gf_expr_type)va_arg(args, int);
}

void gf_error(void *scan, parse_context *ctxt, const char* fmt, ...)
{
  va_list args;
  va_start(args, fmt);
  gf_verror(scan, ctxt, fmt, args);
  va_end(args);
}

void gf_verror(void *scan, parse_context *ctxt, const char*fmt, va_list args)
{
  ...
  ctxt->last_line = gf_get_lineno(scan);
}

const char *gf_expr_type_name(gf_expr_type e)
{
  switch(e) {
  case gf_expr_number: return "Number";
  case gf_expr_vector: return "Vector";
  case gf_expr_string: return "String";
  case gf_expr_var:    return "Variable";
  case gf_expr_config: return "Config";
  case gf_expr_elem:   return "Element";
  case gf_expr_line:   return "Beamline";
  case gf_expr_invalid:return "Invalid";
  default: return "Unknown_expression_type";
  }
}

void gf_string_debug(FILE *fp, const string_t *s)
{
  fprintf(fp, "%s", s->str.c_str());
}

void gf_expr_debug(FILE *fp, const expr_t *E)
{
  fprintf(fp, "%p type %s", (const void *)E, gf_expr_type_name(E->etype));
  if(E->etype==gf_expr_line) {
    try{
...
string_t* gf_string_alloc(const char *s, size_t n)
{
  try{
...
void gf_string_cleanup(string_t* str)
{
  delete str;
}

void gf_expr_cleanup(expr_t* expr)
{
  delete expr;
}

void gf_vector_cleanup(vector_t* vec)
{
  delete vec;
}

void gf_kvlist_cleanup(kvlist_t* pval)
{
  delete pval;
}

void gf_strlist_cleanup(strlist_t* pval)
{
  delete pval;
}

kvlist_t* gf_append_kv(parse_context *ctxt, kvlist_t* L, kv_t* V)
{
  ...
  if(!L) {
    try{
      ...
      gf_error(ctxt->scanner, ctxt, "Allocation failure");
      return NULL;
    }
  }
...
strlist_t* gf_append_expr(parse_context *ctxt, strlist_t* list, expr_t *expr)
{
  assert(expr);
  ...
  try{
    ...
    switch(expr->etype) {
    case gf_expr_elem:
      // prepend a single element (append, the result will be reversed in
      // gf_add_line)
	...
	std::vector<std::string>& N
	  (std::get<std::vector<std::string> >(expr->value));
	...
	std::copy(N.rbegin(),
      ...
      break;
    default:
      gf_error(ctxt->scanner, ctxt, "Lines can not be constructed from %s",
	       gf_expr_type_name(expr->etype));
      L.reset(NULL);
    }
    ...
    gf_error(ctxt->scanner, ctxt,
	     "Error appending to vector: incorrect type %s", name.c_str());
    L.reset(NULL);
  } catch(std::exception& e) {
    gf_error(ctxt->scanner, ctxt, "Error appending to vector: %s", e.what());
    L.reset(NULL);
  }
...
vector_t* gf_append_vector(parse_context *ctxt, vector_t *list, expr_t *expr)
{
  assert(expr);
  try{
    std::unique_ptr<expr_t> E(expr);
    ...
    if(expr->etype==gf_expr_number) {
      V->value.push_back(std::get<double>(expr->value));
    } else {
      std::ostringstream strm;
      strm << "Vector element types must be scalar not type "
	   << gf_expr_type_name(expr->etype);
...
expr_t *gf_add_value(parse_context *ctxt, gf_expr_type t, ...)
{
  ...
  try {
	...
	if((it=ctxt->var_idx.find(name->str))!=ctxt->var_idx.end()) {
	   ...
	   */
	  gf_error(ctxt->scanner, ctxt,
		   "Variable/Label '%s' referenced before definition",
		   name->str.c_str());
	  ret.reset();
	}
      }
    ...
    default:
      gf_error(ctxt->scanner, ctxt, "Logic error, type=%d", (int)t);
      ret.reset();
      break;
    }
    va_end(args);
    ...
    gf_error(ctxt->scanner, ctxt, "Exception while constructing value: %s",
	     e.what());
    return NULL;
  }
  return ret.release();
...
static
std::string gf_describe_op(const operation_t* op)
{
  std::ostringstream strm;
  strm << gf_expr_type_name(op->result_type)
       << " '" << op->name <<"'(";
  for(unsigned i=0; i<op->arg_types.size(); i++) {
    if(i>0)
      strm<<", ";
    strm<<gf_expr_type_name(op->arg_types[i]);
  }
  strm << ")";
  return strm.str();
...
expr_t *gf_add_op
(parse_context *ctxt, string_t *name, unsigned N, expr_t **args)
{
  std::unique_ptr<string_t> SN(name);
  std::unique_ptr<expr_t> ret;
  try{

    parse_context::operations_iterator_pair
      opit = ctxt->operations.equal_range(name->str);
	...
	if(i>0)
	  strm<<", ";
	strm<<gf_expr_type_name(args[i]->etype);
      }
      strm << ")";
      gf_error(ctxt->scanner, ctxt, "%s", strm.str().c_str());
	...
	gf_error(ctxt->scanner, ctxt, "User exception evaluating '%s': %s",
		 gf_describe_op(op).c_str(), e.what());
	ret.reset(0);
      }
    ...
    gf_error(ctxt->scanner, ctxt, "Exception evaluating expression op '%s': %s",
	     name->str.c_str(), e.what());
    ret.reset(0);
  }

  while(N) gf_expr_cleanup(args[--N]);
  return ret.release();
}


void gf_assign(parse_context *ctxt, string_t *name, expr_t*value)
{
  assert(name);
  assert(value);
  try{
	...
	gf_error(ctxt->scanner, ctxt, "Name '%s' already defined",
		 V.name.c_str());
      } else {
    ...
    gf_error(ctxt->scanner, ctxt, "Variable '%s' assignment error; %s",
	     name->str.c_str(), e.what());
  }
}

void gf_add_element
(parse_context *ctxt, string_t *label, string_t *etype, kvlist_t *P)
{
  ...
  try{
    ...
    if(label->str.empty() || etype->str.empty()) {
      gf_error(ctxt->scanner, ctxt, "Element with null label or type");

    } else if(ctxt->element_idx.find(label->str)!=ctxt->element_idx.end()) {
      gf_error
	(ctxt->scanner, ctxt, "Name '%s' already used", label->str.c_str());

    } else {
      ctxt->elements.push_back
	(parse_element(label->str, etype->str, props->map));
      ctxt->element_idx[label->str] = ctxt->elements.size()-1;
    }

  } catch(std::exception& e) {
    gf_error(ctxt->scanner, ctxt, "Element '%s' definition error; %s",
	     label->str.c_str(), e.what());
  }
}

void gf_add_line
(parse_context *ctxt, string_t *label, string_t *etype, strlist_t *N)
{
  std::unique_ptr<string_t> SL(label), SE(etype);
  try{
    ...
    if(strcasecmp(etype->str.c_str(),"LINE")!=0) {
      ...
      gf_error(ctxt->scanner, ctxt, "Name '%s' already used",
	       label->str.c_str());
    ...
    gf_error(ctxt->scanner, ctxt, "Line '%s' definition error; %s",
	     label->str.c_str(), e.what());
  }
}

void gf_command(parse_context* ctxt, string_t *kw)
{
  std::unique_ptr<string_t> SK(kw);
  if(strcmp(kw->str.c_str(), "END")!=0) {
    gf_error(ctxt->scanner, ctxt, "Undefined command '%s'", kw->str.c_str());
  }
}

void gf_call1(parse_context *ctxt, string_t *func, expr_t *arg)
{
  std::unique_ptr<string_t> FN(func);
  std::unique_ptr<expr_t> ARG(arg);

  if(func->str!="print") {
    gf_error(ctxt->scanner, ctxt, "Undefined global function '%s'",
	     func->str.c_str());
  } else if(ctxt->printer) {
    std::ostream& strm = *ctxt->printer;
    strm<<"On line "<<gf_get_lineno(ctxt->scanner)<<" : ";
    switch(arg->etype)
      {
      case gf_expr_number: strm<< std::get<double>(arg->value); break;
      case gf_expr_string: strm<<"\""<< std::get<std::string>(arg->value)<<"\"";
	break;
      case gf_expr_vector: {
	std::ostream_iterator<double> it(strm, ", ");
	const std::vector<double>& vect
	  = std::get<std::vector<double> >(arg->value);
	strm<<"[";
	...
	const std::vector<std::string>& vect
	  = std::get<std::vector<std::string> >(arg->value);
	strm<<"(";
	std::copy(vect.begin(), vect.end(), it);
	strm<<")";
      }
	break;
      default:
	strm<<"?? <"<<gf_expr_type_name(arg->etype)<<"> ??";
      }
    strm<<"\n";
  }
}
...
namespace {
  void parse_common
  (YY_BUFFER_STATE (*setup)(yyscan_t, void*), parse_context* ctxt, void *pvt)
  {
    yyscan_t scanner;

    if(gf_lex_init_extra(ctxt, &scanner))
  ...
  {
    FILE *fp = (FILE*)pvt;
    YY_BUFFER_STATE ret = gf__create_buffer(fp, 1024, scanner);
    gf__switch_to_buffer(ret, scanner);
    return ret;
  }
  ...
  YY_BUFFER_STATE setup_string(yyscan_t scanner, void* pvt)
  {
    ...
    return gf__scan_bytes(str->c_str(), str->size(), scanner);
  }
  ...
  YY_BUFFER_STATE setup_bytes(yyscan_t scanner, void* pvt)
  {
    ...
    return gf__scan_bytes(buf->bytes, buf->len, scanner);
  }
}
...
void
parse_context::addop
(const char *name, operation_t::eval_t fn, gf_expr_type R, unsigned N, ...)
{
  va_list args;
  va_start(args, N);
  try{
    operations.insert
      (std::make_pair(name, operation_t(name, fn, R, N, args)));
