...
namespace detail {
  ...
  // helper to ensure that attempts to call Config::get<T> for unsupported T
  // will fail to compile.
  template<typename T>
  struct is_config_value {
  };
#define IS_CONFIG_VALUE(TYPE)					\
  namespace detail {template<> struct is_config_value<TYPE>     \
 { typedef TYPE type; };}
} // namespace detail
...
public:
  ...
  /** lookup typed.
   * @throws key_error if name doesn't refer to an existing parameter, or the
   * parameter has other than the requested type.
   *
   @code
   Config X;
  ...
  */
  ...
  get(const std::string& name) const {
    try {
      ...
      throw key_error
	(SB()<<"Wrong type for '"<<name<<"'.  should be "<<typeid(T).name());
    }
  }
   ...
   *
   ...
   Config X;
   double v =
   X.get<double>("param", 0.0); // returns 0.0 since "param" is not set
   @endcode
  */
  template<typename T>
  typename detail::RT<T>::type
  get(const std::string& name, typename boost::call_traits<T>::param_type def)
    const {
    try{
      return std::get<typename detail::is_config_value<T>::type>(getAny(name));
    } catch(std::bad_variant_access&) {
  ...
  */
   ...
   <typename detail::is_config_value<T>::type>::reference val)
  {
    ...
    swapAny(name, temp);
    //val.swap(std::get<T>(temp));
    // TODO: detect insert, make this a real swap
  }
...
public:
   ...
   * @args path The directory containing the open file (used to expand relative
   * paths in the file)
...
void GFPrint(std::ostream& strm, const Config&);
